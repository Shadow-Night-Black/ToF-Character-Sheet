            ...
            <table className="skill-grid table-condensed">
                <thead className="skill-header">
                    <tr>
                        ...
                        {editMode ? (
                            <th className="skill-attribute" colSpan={attributes.length}>
                                Attributes
                            </th>
                        ) : <th className="skill-attribute">Total Level</th>}
                    ...
                    {editMode ? (
                        <tr>
                            ...
                            {attributes.map((a) => {
                                return (
                                    <th className={`skill-attribute ${a.name}`}>
                                        {a.name}
                                    </th>
                                );
                            })}
                            <th className="skill-dice"> </th>
                            <th className={'skill-delete'}> </th>
                        </tr>
                    ) : null}
                </thead>
                <tbody>
                    {state.skills.map((skill) =>
                        editMode ? EditBodyRow(skill, skillLens, attributes) : WidgetBodyRow(skill)
                    )}
                </tbody>
            </table>
            {editMode ? (
...
function EditBodyRow(skill: Skill, { update, remove }: ListAccessors<Skill>, attributes: Attribute[]): JSX.Element {
    const updateSkill = (newValues: Partial<Skill>) => {
        const newSkill = NewSkill({ ...skill, ...newValues });
        if (SkillValidator(newSkill)) update(skill, newSkill);
    };
    ...
    return (
        <tr className={`skill-row`} key={skill.id}>
            ...
            {attributes.map((a) => {
                return (
                  <td className={`skill-level ${a.name}`}>
                        <input
                            type="number"
                            max={MaxSkillLevel}
                            min={MinSkillLevel}
