...
export function CreateTestCharacter(): Character {
  ...
  char.totem.blessings = GetDefaultBlessings()
    .filter((x) => char.totem.fated.map((x) => x.key).includes(x.fated.key))
    .slice(0, 5);
...
export function CharacterToJson(character: Character): string {
  return JSON.stringify({
    version: 1,
    data: { ...character, attributes: GetDefaultAttributes().map((x) => character.attributes.get(x)) }
  });
}
...
export const UpdateSkillList: UpdateMember<Skill[], Character> = (map) => (old) => ({ ...old, skills: map(old.skills) });
